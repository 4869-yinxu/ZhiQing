...
def login(request):
    ...
    with connection.cursor() as cursor:
        ...
        if not results:
            print(f"[DEBUG] 用户 {username} 不存在或状态异常")
            return JsonResponse(
        ...
        print(f"[DEBUG] 密码验证成功，用户ID: {results[0].get('id')}")
