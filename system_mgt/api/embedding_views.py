...
def embedding_test(request, embedding_id):
    ...
    try:
        ...
        logger.info(f"测试嵌入模型 {embedding_id}，测试文本: {test_text}")
        ...
        if not model_config:
            logger.error(f"模型 {embedding_id} 不存在")
            return JsonResponse(
                ResponseCode.ERROR.to_dict(message="模型不存在或无权限操作"),
                status=404
            )
        
        logger.info(f"模型类型: {model_config.get('api_type')}, 用户权限: role_id={role_id}, user_id={user_id}")
        if not model_config:
            return JsonResponse(
                ResponseCode.ERROR.to_dict(message="模型不存在或无权限操作"),
                status=404
        ...
        # 检查用户权限
        logger.info(f"权限检查: role_id={role_id}, model_user_id={model_config['user_id']}, is_public={model_config.get('is_public')}")
        if role_id != 1 and model_config['user_id'] != user_id and not model_config.get('is_public'):
            logger.warning(f"用户 {user_id} 无权限操作模型 {embedding_id}")
            return JsonResponse(
                ResponseCode.ERROR.to_dict(message="无权限操作此模型"),
                status=403
            )
        
        logger.info(f"权限检查通过，开始测试模型")
        ...
        if model_config['api_type'] == 'local':
            ...
            # 使用LlamaIndex测试OpenAI兼容模型
            try:
                from system_mgt.utils.llms_manager import llms_manager
                
                # 初始化OpenAI兼容模式Embedding模型
                success = llms_manager.initialize_embedding("openai_like", model_config)
                if not success:
                    return JsonResponse(
                        ResponseCode.ERROR.to_dict(message="初始化OpenAI兼容模式Embedding模型失败"),
                        status=500
                    )
                
                # 测试Embedding功能
                test_result_data = llms_manager.test_embedding(test_text)
                if not test_result_data.get('success'):
                    return JsonResponse(
                        ResponseCode.ERROR.to_dict(message=f"OpenAI兼容模式Embedding测试失败: {test_result_data.get('error')}"),
                        status=500
                    )
                
                vector = test_result_data['embedding']
                actual_dimension = test_result_data['dimension']
                ...
                logger.error(f"OpenAI兼容模式Embedding测试失败: {str(e)}")
                return JsonResponse(
                    ResponseCode.ERROR.to_dict(message=f"OpenAI兼容模式Embedding测试失败: {str(e)}"),
                    status=500
                )
                
        elif model_config['model_type'] == 'sentence_transformers':
            # 使用LlamaIndex测试Sentence Transformers本地模型
            try:
                from system_mgt.utils.llms_manager import llms_manager
                
                # 初始化Sentence Transformers Embedding模型
                success = llms_manager.initialize_embedding("sentence_transformers", model_config)
                if not success:
                    return JsonResponse(
                        ResponseCode.ERROR.to_dict(message="初始化Sentence Transformers Embedding模型失败"),
                        status=500
                    )
                ...
                if not test_result_data.get('success'):
                    return JsonResponse(
                        ResponseCode.ERROR.to_dict(message=f"Sentence Transformers Embedding测试失败: {test_result_data.get('error')}"),
                        status=500
                    )
                ...
                logger.error(f"Sentence Transformers Embedding测试失败: {str(e)}")
                return JsonResponse(
                    ResponseCode.ERROR.to_dict(message=f"Sentence Transformers Embedding测试失败: {str(e)}"),
                    status=500
                )
                
        elif model_config['api_type'] == 'ollama':
            # 使用LlamaIndex测试Ollama模型
            try:
                from system_mgt.utils.llms_manager import llms_manager
                
                # 初始化Ollama Embedding模型
                success = llms_manager.initialize_embedding("ollama", model_config)
                if not success:
                    return JsonResponse(
                        ResponseCode.ERROR.to_dict(message="初始化Ollama Embedding模型失败"),
                        status=500
                    )
                ...
                if not test_result_data.get('success'):
                    return JsonResponse(
                        ResponseCode.ERROR.to_dict(message=f"Ollama Embedding测试失败: {test_result_data.get('error')}"),
                        status=500
                    )
                ...
                logger.error(f"Ollama Embedding测试失败: {str(e)}")
                return JsonResponse(
                    ResponseCode.ERROR.to_dict(message=f"Ollama Embedding测试失败: {str(e)}"),
                    status=500
                )
                
        elif model_config['api_type'] == 'zhipuai':
            # 使用LlamaIndex测试智谱AI模型
            try:
                from system_mgt.utils.llms_manager import llms_manager
                
                # 初始化智谱AI Embedding模型
                success = llms_manager.initialize_embedding("zhipuai", model_config)
                if not success:
                    return JsonResponse(
                        ResponseCode.ERROR.to_dict(message="初始化智谱AI Embedding模型失败"),
                        status=500
                    )
                ...
                if not test_result_data.get('success'):
                    return JsonResponse(
                        ResponseCode.ERROR.to_dict(message=f"智谱AI Embedding测试失败: {test_result_data.get('error')}"),
                        status=500
                    )
                ...
                logger.error(f"智谱AI Embedding测试失败: {str(e)}")
                return JsonResponse(
                    ResponseCode.ERROR.to_dict(message=f"智谱AI Embedding测试失败: {str(e)}"),
                    status=500
                )
                
        elif model_config['api_type'] == 'baichuan':
            # 使用LlamaIndex测试百度千帆模型
            try:
                from system_mgt.utils.llms_manager import llms_manager
                
                # 初始化百度千帆 Embedding模型
                success = llms_manager.initialize_embedding("baichuan", model_config)
                if not success:
                    return JsonResponse(
                        ResponseCode.ERROR.to_dict(message="初始化百度千帆 Embedding模型失败"),
                        status=500
                    )
                ...
                if not test_result_data.get('success'):
                    return JsonResponse(
                        ResponseCode.ERROR.to_dict(message=f"百度千帆 Embedding测试失败: {test_result_data.get('error')}"),
                        status=500
                    )
                ...
                logger.error(f"百度千帆 Embedding测试失败: {str(e)}")
                return JsonResponse(
                    ResponseCode.ERROR.to_dict(message=f"百度千帆 Embedding测试失败: {str(e)}"),
                    status=500
                )
                
        elif model_config['api_type'] == 'dashscope' or (model_config['api_type'] == 'online' and model_config['model_type'] == 'dashscope'):
            # 使用LlamaIndex测试DashScope模型
            logger.info(f"开始测试dashscope类型模型: {model_config['name']}")
            try:
                from system_mgt.utils.llms_manager import llms_manager
                
                # 初始化DashScope Embedding模型
                success = llms_manager.initialize_embedding("dashscope", model_config)
                if not success:
                    return JsonResponse(
                        ResponseCode.ERROR.to_dict(message="初始化DashScope Embedding模型失败"),
                        status=500
                    )
                ...
                if not test_result_data.get('success'):
                    return JsonResponse(
                        ResponseCode.ERROR.to_dict(message=f"DashScope Embedding测试失败: {test_result_data.get('error')}"),
                        status=500
                    )
                ...
                logger.error(f"DashScope Embedding测试失败: {str(e)}")
                return JsonResponse(
                    ResponseCode.ERROR.to_dict(message=f"DashScope Embedding测试失败: {str(e)}"),
                    status=500
                )
