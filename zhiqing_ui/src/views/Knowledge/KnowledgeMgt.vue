...
const fetchList = async () => {
  ...
  try {
    ...
    // 修复数据结构匹配问题
    // 由于axios响应拦截器已经返回了response.data，所以这里直接使用data
    if (data && data.list) {
      // 数据结构: { list: [...], total: 5, page: 1, page_size: 20 }
      tableData.data = data.list || [];
      tableData.total_records = data.total || 0;
    } else if (data && data.data && data.data.list) {
      // 兼容嵌套数据结构: { data: { list: [...], total: 1 } }
      tableData.data = data.data.list || [];
      tableData.total_records = data.data.total || 0;
    } else {
      // 兼容旧的数据结构
      tableData.data = data.data || [];
      tableData.total_records = data.total_records || 0;
    }
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`确认删除知识库 "${row.name}"？删除后不可恢复！`, "警告", {
    ...
    .then(async () => {
      loading.value = true;
      try {
        await axios.delete(`knowledge/database/${row.id}/delete/`);
  ...
  <div class="knowledge-manager-container">
    ...
    <div class="container" v-loading="loading">
      ...
      <el-table :data="tableData.data" border>
        <!-- 调试信息 -->
        <div v-if="tableData.data.length === 0" style="padding: 20px; text-align: center; color: #999;">
          暂无数据 (总数: {{ tableData.total_records }}, 数据: {{ tableData.data.length }})
        </div>
