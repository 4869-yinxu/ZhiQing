...
class DocumentProcessor:
    ...
    def split_text(self, text):
        ...
        if self.chunking_method == "token":
            ...
            chunks = self._split_by_semantic_improved(text)
    ...
    def _split_by_chapter(self, text):
        ...
        # 尝试使用专门的章节处理器
        try:
            from .document_processor.chapter_processor import ChapterProcessor
            chapter_processor = ChapterProcessor(self.config)
            chunks = chapter_processor.split_text(text)
            if chunks and len(chunks) > 1:
                logger.info(f"使用专门章节处理器，生成 {len(chunks)} 个分块")
                return chunks
        except Exception as e:
            logger.debug(f"专门章节处理器不可用，使用内置方法: {e}")
        
        # 回退到内置的章节分块方法
        return self._split_by_chapter_builtin(text)
    
    def _split_by_chapter_builtin(self, text):
        """内置章节分块方法"""
        # 检测标题模式：# ## ### 或 第一章 第二节 等
        title_patterns = [
            r'^#{1,6}\s+.+$',  # Markdown标题
    ...
    def _split_by_semantic_improved(self, text):
        """改进的语义分块方法"""
        # 尝试使用专门的语义处理器
        try:
            from .document_processor.semantic_processor import SemanticProcessor
            semantic_processor = SemanticProcessor(self.config)
            chunks = semantic_processor.split_text(text)
            if chunks and len(chunks) > 1:
                logger.info(f"使用专门语义处理器，生成 {len(chunks)} 个分块")
                return chunks
        except Exception as e:
            logger.debug(f"专门语义处理器不可用，使用内置方法: {e}")
        
        # 回退到内置的语义分块方法
        return self._split_by_semantic_builtin(text)
    
    def _split_by_semantic_builtin(self, text):
        """内置语义分块方法"""
        # 先按句子分割
        sentences = re.split(r'[.!?。！？；;]\s*', text)
        sentences = [s.strip() for s in sentences if s.strip()]
